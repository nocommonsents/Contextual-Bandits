...
class EnsembleTestModel4(ContextualBanditPolicy):
    ...
    def getActionToPerform(self, visitor, possibleActions):
        ...
        policy_three_scaled_values = self.policy_three.getCTRs(visitor, possibleActions)
        ...
        policy_three_scores_list = [self.policy_three_scores[vis.getID()] for vis in possibleActions]

        max_policy_one_score = max(policy_one_scores_list)
        max_policy_two_score = max(policy_two_scores_list)
        delta = abs(float(max_policy_one_score-max_policy_two_score))
        ...
        elif (max_policy_two_score > max_policy_one_score and float(delta)/max_policy_one_score > 0.1):
            #print "Here2 " + str(max_policy_one_score) + " " + str(max_policy_two_score)
