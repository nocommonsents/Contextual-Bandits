...
# BinomialUCI base algorithm from https://www.ki.tu-berlin.de/fileadmin/fg135/publikationen/Seiler_2013_MLR.pdf, page 8
...
class EnsembleBinomialUCIMod1(ContextualBanditPolicy):
    ...
    def __init__(self, regressor):
        ...
        self.policy_most_recent_runtimes = {}
        ...
        for i in self.policies:
            self.policy_runtimes[str(i)] = 0.001
            self.policy_counts[str(i)] = 1.0
            self.policy_successes[str(i)] = 1.0
            self.policy_ucis[str(i)] = 1.0
            self.policy_runtime_to_count_ratios[str(i)] = 0.001
            self.policy_most_recent_runtimes[str(i)] = 0.001
    ...
    def updatePolicy(self, content, chosen_arm, reward, *possibleActions):
        ...
        self.policy_most_recent_runtimes[str(self.chosen_policy)] = elapsed_time
        ...
        if (self.updates % 100 == 0):
            ...
            for i in self.policies:
                ...
                print str("EnsembleBinomialUCIMod1") + "," + str(self.policy_nicknames[self.policies.index(i)]) + "," + \
                      str(self.updates) + "," + str(float(self.policy_counts[str(i)])/self.updates)
                output_file.write(str("EnsembleBinomialUCIMod1") + "," + str(self.policy_nicknames[self.policies.index(i)]) + ","
                                  + str(self.updates) + "," + str(float(self.policy_counts[str(i)])/self.updates) + "\n")
            #print "\n"